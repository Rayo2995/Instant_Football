...
<main class="modulo-wrap">
  ...
  <div class="modulo-header">
    <div>
      ...
      <p class="modulo-sub">Clasificación actualizada automáticamente</p>
    </div>
  </div>
  ...
  <?php if (empty($tabla)): ?>
    <div class="empty-state glass">
      ...
      <a href="../partidos/crear.php" class="btn-red">Registrar partido</a>
    </div>
  <?php else: ?>
      ...
      <table class="tabla">
        ...
        <tbody>
          <?php foreach ($tabla as $i => $eq): 
            ...
            $dgClass = $dg > 0 ? 'dg-pos' : ($dg < 0 ? 'dg-neg' : 'dg-neu');
          ?>
            <tr class="<?= $pos === 1 ? 'fila-lider' : ($pos <= 3 ? 'fila-top' : '') ?>">
              ...
              <td class="td-nombre">
                <?php if ($eq['escudo']): ?>
                  <img src="/Instant_Football/img/escudos/<?= htmlspecialchars($eq['escudo']) ?>"
                       style="width:24px;height:24px;border-radius:6px;object-fit:cover;vertical-align:middle;margin-right:0.5rem;">
                <?php endif; ?>
                <?= htmlspecialchars($eq['equipo']) ?>
              </td>
              <td><?= $eq['partidos_jugados'] ?></td>
              <td class="td-v"><?= $eq['victorias'] ?></td>
              <td><?= $eq['empates'] ?></td>
              <td class="td-d"><?= $eq['derrotas'] ?></td>
              <td><?= $eq['goles_favor'] ?></td>
              <td><?= $eq['goles_contra'] ?></td>
              <td><span class="dg <?= $dgClass ?>"><?= $dg > 0 ? '+' : '' ?><?= $dg ?></span></td>
              <td><span class="pts"><?= $eq['puntos'] ?></span></td>
            </tr>
          <?php endforeach; ?>
        </tbody>
      </table>
    ...
    <div class="tabla-leyenda">
      <span class="leyenda-item"><span class="leyenda-dot dot-lider"></span> Líder</span>
      <span class="leyenda-item"><span class="leyenda-dot dot-top"></span> Top 3</span>
      <span class="leyenda-sep">·</span>
      <span class="leyenda-info">PJ: Partidos Jugados · V: Victorias · E: Empates · D: Derrotas · GF: Goles Favor · GC: Goles Contra · DG: Diferencia · Pts: Puntos</span>
    </div>
